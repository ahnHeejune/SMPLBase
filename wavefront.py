...
def save_obj2(filename: str, uvfilename: str, vers, faces, texcoords ):
    ...
    with open( filename, 'w' ) as ofile:
        ...
        for i in range(vertices.shape[0]):
             ofile.write('v '+' '.join(['{}'.format(vertices[i,j]) for j in range(vertices.shape[1]))+'\n')       
        for i in range(texcoords.shape[0]):
            ofile.write('vt '+' '.join(['{}'.format(texcoords[i,j]) for j in range(texcoords.shape[1]))+'\n')
...
if __name__ == "__main__": 
    ...
    obj = load_obj( '..\pyrender\examples\models\drill.obj' )

    save_obj( obj, 'copy.obj' )
