    ...
    for o_id, orient in enumerate(orientations):
        ...
        if viz:
            ...
            def on_step(_):  
                ...
                rendered = (render_model(
                    sv.r, model.f, w, h, cam, far=20 + dist, img =img[:,:,::-1]) * 255./1.0).astype('uint8') # background 
        ...
        for stage, (w, wbetas) in enumerate(opt_weights):
            ...
            b, p = ch.minimize(
                       ...
                       method='dogleg',
                       callback=on_step,
                       options={'maxiter': 10, 'e_3': .0001, 'disp': 0})
        ...
        if try_both_orient:
            errors.append((objs['j2d'].r**2).sum())   # only compare the joint distances
        svs.append(sv)
        cams.append(cam)

    if try_both_orient and errors[0] > errors[1]:  # choose best from both orientation.
        choose_id = 1
    else:
        choose_id = 0
    if viz:
...
def run_single_fit(img,
    ...
    for deg in do_degrees:
        ...
        im = (render_model(
            verts, model.f, w, h, cam, far=20 + dist, img=img[:,:,::-1]) * 255./1.0).astype('uint8')
        images.append(im)
...
if __name__ == '__main__':
    ...
    args.dataset = 'beforeafter' #'10k'
    args.base_dir = join(join(args.base_dir, 'dataset'), args.dataset) # base dataset directory 
